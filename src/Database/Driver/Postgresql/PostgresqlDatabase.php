...
class PostgresqlDatabase extends AbstractDatabase
{
	...
	public function create($ifNotExists = false, $charset = null, $collate = null)
	{
		if ($ifNotExists)
		{
			$result = $this->db->setQuery("SELECT 1 from pg_database WHERE datname='{$this->database}'")->loadResult();
		...
		$query = PostgresqlQueryBuilder::createDatabase($this->database, false, $charset, $collate);
